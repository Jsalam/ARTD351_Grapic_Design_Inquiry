var cLuminance = function (p5){
	...
	p5.setupOutputLimits = function(minOutput, maxOutput){
		...
		for (var i=0; i< colors.length; i++){
			...
			colors[i].setLuminance(p5.scale.minOutput + (i * step));
		}
	}
	...
	p5.assignInputValue = function (val){
		...
		colorOutput.setLuminance(p5.scale.mappedVal);
	}
	...
	var MyColor = function (p5, x, y, wdt, hght){
		...
		this.setLuminance = function (val){

			if (val >=0 && val <1 ){

				var tmp = this.myChroma.luminance(val);

				this.myColor = p5.color(tmp.get('rgb.r'), tmp.get('rgb.g'),tmp.get('rgb.b'));
				
			} else if (val >= 1) {

				var tmp = this.myChroma.luminance(0.99);
...
var cLuminanceInstance = new p5(cLuminance, 'cLuminance');